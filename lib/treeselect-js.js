...
class Treeselect {
    ...
    #createTreeselect() {
        ...
        n.srcElement.addEventListener("input", e=>{
            var ids = e.detail.map(({id: e})=>e);
            //e = e.detail.map(({id: e})=>e);
            //this.value = e,
            let tmpVals = [];
            this.#treeselectList.flattedOptions.map(opItem => {
                if(ids.find(idItem => idItem == opItem.childOf)){
                    tmpVals.push(opItem)
                }
            })
            console.log('inputing',tmpVals,  e.detail, this.value, this.options, this.#treeselectList.flattedOptions);
            const valIds = tmpVals.map(({id: i}) => i);
            this.value = valIds
            s.updateValue(valIds),
            this.#emitInput(tmpVals);
        }
        ),
           ...
           e = [];
            t.map(tItem => {
                if(tItem.isGroup && tItem.childOf == 0){
                    ...
                    const exist = e.find(item => item.id == tItem.childOf);
                    if(!exist){
